...
function SplashCursor({
  ...
  CURL = 3,
  SPLAT_RADIUS = 0.1, // reduced for subtler effect
  SPLAT_FORCE = 3000, // reduced for less intensity
  SHADING = true,
  ...
  useEffect(() => {
    ...
    if (window.matchMedia && window.matchMedia("(pointer: coarse)").matches) {
      // Skip heavy effect on touch devices
      return;
    }
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      let gl: any = canvasEl.getContext("webgl2", params);
      const isWebGL2 = !!gl;
    ...
    function drawDisplay(target: any) {
      ...
      let height = target == null ? target?.height ?? gl.drawingBufferHeight : target.height;
    ...
    function updateFrame() {
      ...
      step(dt);
      render(null);
      animationFrameId.current = window.requestAnimationFrame(updateFrame);
    }
